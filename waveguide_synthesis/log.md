...
## [2026-09-10] ingest | DAFx26 proceedings - pass 2 (deep distillation into concept pages)
- Read the full text extractions of paper_04, paper_05, paper_10, paper_23, paper_35, paper_36, paper_38, paper_39, paper_43, demo_61, demo_68 and demo_73 from /l/dttd/DAFx26/txt/.
- Two papers were missing from the pass-1 catalog and were added: paper_35 (Essl, winding numbers and monodromy over a circular buffer) and demo_73 (Wieland, Roth & Studer, PolyMap 64-channel guitar pickup). Their .txt extractions were copied into raw/, with new MANIFEST.md rows and a "pass-2 additions" block in raw/SUMMARIES.md, plus catalog sections under a new "Theory, topology and measurement" heading in entities/source-papers.md.
- Created 9 concept pages: fdn-kronecker-feedback-matrices (recursive 2x2 Kronecker kernels, O(N log N) feedback multiply, stereo cross-coupling, selective freeze, resonance-breaking modulation); nonlinear-and-time-varying-fdn-effects (five shimmer nonlinearities in the FDN loop with energy and stability analysis); differentiable-fdn-design (frequency-sampling differentiability, proportional PEQ attenuation, composite EDR loss, ADAC export and stability certificate); differentiable-karplus-strong (tKSA vs fKSA, Lagrange delay gradients, the onset-gradient failure, parameter loss); differentiable-kelly-lochbaum-tract (sublingual source, B-spline area function, learnable damping, 30-38% LSD reduction); loopback-fm-time-varying-delay (TVDL time-warping relation, LBFM closed form, delay wrapping and offset); bowed-string-finite-difference-models (yehu: modal bridge, elastic bow hairs, energy quadratisation, elasto-plastic friction, 6.6x real time); wdf-code-generation (declarative DSL, static compiler, benchmarks vs RT-WDF / wdmodels / chowdsp_wdf, performance score); circular-buffer-topology-monodromy (line bundles, Mobius monodromy, winding numbers, spectral consequences, braids).
- Created 2 entity pages: sffdn-library (sfFDN + FDN Sandbox, ensmallen optimizers, colorless and RIR-matching losses) and polymap-pickup-system (8x8 pickup grid, MADI transport, 5.4 ms latency, comb-filter pickup-position measurements).
- Added one-line "see also" pointers and bumped `updated` on artificial-reverberation, delay-line-techniques, string-modeling, waveguide-parameter-optimization, wdf-applications, wave-digital-filters, reed-and-bow-models, waveguide-vocal-models, scattering-junctions and waveguide-overview.
- All new pages are under 100 lines per SCHEMA, with LaTeX math, a footnote citation to the source-paper catalog anchor, and at least two resolving outbound wikilinks.
- Updated index.md: 33 pages, 16 DAFx26 source-paper anchors.
