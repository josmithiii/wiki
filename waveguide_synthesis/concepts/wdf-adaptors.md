...
Junction force from incoming waves:
  f_J = 2 * (Gamma_1 * f_1^+ + Gamma_2 * f_2^+) / (Gamma_1 + Gamma_2)
where Gamma_i = 1/R_i.
...
Junction force:
  f_J = 2 * sum(Gamma_i * f_i^+) / sum(Gamma_i)
Outgoing waves:
  f_i^- = f_J - f_i^+

When all impedances are equal (Gamma_i = Gamma for all i):
  f_J = (2/N) * sum(f_i^+) — multiply-free when N is power of 2

### N-Port Series (velocity waves)
Junction velocity:
  v_J = 2 * sum(R_i * v_i^+) / sum(R_i)
...
port impedance is computed from the others to eliminate one multiply:
  R_dep = function of all other R_i
