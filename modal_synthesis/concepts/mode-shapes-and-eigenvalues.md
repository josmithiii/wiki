...
Drop damping: M * x'' + K * x = 0
Seek solutions x(t) = phi * exp(j*omega*t):
  K * phi = omega^2 * M * phi   (generalized eigenvalue problem)

Solutions: N pairs (omega_k^2, phi_k)
- omega_k = 2*pi*f_k = kth natural frequency (rad/s)
- phi_k = kth mode shape (eigenvector)

## Mode Shape Properties
- Orthogonality: phi_i^T * M * phi_j = 0 for i != j
...
where q_k(t) are modal coordinates satisfying decoupled 2nd-order ODEs:
  q_k''(t) + 2*d_k*q_k'(t) + omega_k^2 * q_k(t) = phi_k^T * f(t)
...
Output pressure at microphone position r_m:
  p(t) ~ sum_k [ phi_k(r_excite) * phi_k(r_pickup) * q_k(t) ]

- phi_k(r_excite) = mode shape at excitation point → determines input coupling
- phi_k(r_pickup) = mode shape at pickup point → determines output coupling
