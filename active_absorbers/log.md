...
## [2026-04-22] ingest | community-health LFN singleton — Erdélyi et al. 2023 IJERPH
- Source: `/l/dttd/LowFrequencyNoise-ijerph-20-03916.pdf` (already symlinked in `/l/dttd/ANC-Stuff/` by JOS on 2026-04-22). Open-access CC BY 4.0; DOI 10.3390/ijerph20053916.
- Extracted via `pdf2txt.py` → `raw/LowFrequencyNoise-ijerph-20-03916.txt` (2425 lines).
- **Not an ANC methods paper** — first community-health / exposure-response paper in the wiki. Ingested as the "why bother" empirical anchor for the rooftop-fan effort: cross-sectional questionnaire study of $n{=}190$ Dutch LFN-perceivers vs. $n{=}371$ demographically-matched controls. Key findings: air-conditioning/ventilation is the #1 assumed LFN source (20%), perceived frequencies span 5–140 Hz (covers the ≈118 Hz BPF band), self-rated impact on daily life averages $7.1/10$, sleep difficulties reported by 90% of perceivers, and passive mitigation (earplugs, source shutoff, window management, bed position) was rated *unsuccessful* by 72–86% of those who tried each action.
- `raw/MANIFEST.md`: appended row; extraction-date footer amended with "2026-04-22 (LFN-perception singleton)".
- `raw/SUMMARIES.md`: added new "Community-Health LFN Batch (2026-04-22)" section with dense summary.
- `entities/source-papers.md`: added new top-level "Community-health / LFN perception batch (2026-04-22)" section with `paper-erdelyi-lfn-perceptions-2023`. Tags: `psychoacoustic, fan-noise, industrial, reference` (all from existing taxonomy — no SCHEMA tag additions needed).
- `concepts/rooftop-fan-contenders.md`: new **§0 "Why this matters (community-health evidence)"** at top of body, citing Erdélyi 2023. Frontmatter `sources:` extended; new `[^erdelyi-2023]` footnote added.
- `concepts/psychoacoustic-anc.md`: §7 extended with "Empirical corroboration" bullet citing Erdélyi 2023. Frontmatter `sources:` extended; "Sources ingested" section now lists Erdélyi 2023 alongside Rivera 2018; new `[^erdelyi-2023]` footnote added. Took the opportunity to upgrade the existing Rivera reference/footnote from bare-filename form to the proper `[paper-...](../entities/source-papers.md#paper-...)` anchor form so it renders as a working link in the HTML build (matches the rooftop-fan-contenders §4.5 style already in use).
- `index.md`: header bumped to "Last updated: 2026-04-22 | Concepts: 15 · Source papers in catalog: 45" (44 → 45); new "Community-health / LFN perception batch (2026-04-22)" subsection under Entities.

## [2026-06-18] ingest | electroacoustic-absorbers batch — Olson-May 1953, Lissek 2011, Rivet 2017
- Trigger: JOS dropped three PDFs in `~/w/absorbers/papers/` while discussing whether a large electrostatic speaker could absorb ~100 Hz fan blade noise (excursion question). These are the three active-absorption references surfaced in that conversation.
- Host-side ingestion (per SCHEMA): `cp` each PDF to `/l/dttd/` with a clean name, symlink into `/l/dttd/ANC-Stuff/`, `pdf2txt.py` → `raw/`. (Used `cp` not `mv` — originals left in `~/w/absorbers/papers/` since that is JOS's download dir, not canonical store; the `/l/dttd/` copy is canonical.)
  - `Lissek-Boulandet-Fleury-ElectroacousticAbsorbers-JASA2011.txt` (54 kB) and `Rivet-Karkar-Lissek-BroadbandLF-ElectroacousticAbsorbers-2017.txt` (47 kB) extracted cleanly.
  - `Olson-May-ElectronicSoundAbsorber-JASA1953.pdf` is the **declassified CIA reading-room scan** (CIA-RDP78-03300A001900110032-3) — image-only, no text layer. `pdf2txt.py` returned 7 bytes; `ocrmypdf --force-ocr` failed (tesseract/leptonica could not read its own rendered PNGs — libpng issue in this env, both `--jobs 1` and default). Fell back to a **visual read of the 7 page images** (permitted when pdf2txt fails) and hand-wrote a dense structured transcription into `raw/Olson-May-ElectronicSoundAbsorber-JASA1953.txt` (abstract, equations $p_2=Bli/S$, Table I, two operating modes, Figs 1–21, performance, applications).
- New concept page **`concepts/electroacoustic-absorbers.md`** (type concept; 16th concept). Justified by the 100-line split rule (the existing `hybrid-active-passive.md` is already 276 lines) + active-impedance-via-the-diaphragm being central to 2 of the 3 sources. Covers: absorption criterion $\alpha=1-|r|^2$; the 1-DOF model and its three knobs $(R_s,C_v,C_p)$; control families (shunt → velocity-fb → direct impedance control → current-driven feedforward); the $\rho c$ matched-termination result and the **micron-excursion absorption argument** (answers JOS's electrostatic-panel question: ESL excursion limit bites for *radiating* bass, not *absorbing* it); the Rivet bandwidth law; rooftop-fan relevance incl. the aperture-coverage caveat.
- `entities/source-papers.md`: new "## Electroacoustic absorbers batch (2026-06-18)" section with three slug entries — `paper-olson-may-electronic-sound-absorber-1953`, `paper-lissek-boulandet-fleury-electroacoustic-absorbers-2011`, `paper-rivet-karkar-lissek-broadband-lf-ea-2017`. Frontmatter `updated` → 2026-06-18.
- `raw/SUMMARIES.md`: new "Electroacoustic Absorbers Batch (2026-06-18)" section with three catalog pointers.
- `concepts/hybrid-active-passive.md`: §1 now points to the new `[[electroacoustic-absorbers]]` page; Olson & May bullet promoted from "earliest electronic sound absorber" stub to ingested-with-matched-termination detail; "Pending sources" Olson line struck through as ingested; frontmatter `sources:` + `updated` (→2026-06-18) extended; new `[^olson-1953]` footnote.
- `raw/MANIFEST.md`: 3 new rows + extraction-date footer amended with "2026-06-18 (electroacoustic-absorbers batch...)". Olson row documents the OCR failure + visual-read provenance.
...
## [2026-06-19] edit | electroacoustic-absorbers — add "Would an AMT do better?" comparison
- Follow-up to JOS asking whether an Air Motion Transformer beats an ESL for absorption. New `### Would an Air Motion Transformer (AMT) do better?` subsection under §7 with an ESL-vs-AMT table.
- Conclusion: no — the AMT is an open dipole (same 50% cap, §5) and a sub-wavelength $\gtrsim$650 Hz device (same aperture limit, §6); its $5\times$ velocity step-up optimizes excursion, which was never the constraint. Worth borrowing only in a sealed one-port design: fold geometry concentrates velocity (viscous loss $\propto u^2$ → good passive resistive loss) and its high radiation resistance is what Olson/Lissek add electrically.
- Mirror of this comparison added to project-side `~/w/absorbers/Discussion.md` §8.

## [2026-06-19] edit | electroacoustic-absorbers — add the aperture-size (diffraction) energy-flux limit
- Per JOS: the "hole-punch" picture is the *high-frequency* geometric limit (A≫λ², baffled); the rooftop case is diffraction-limited (panel ≪ λ). Added the sub-wavelength energy-flux analysis to §5 (retitled "Two ceilings: the 50% open-dipole cap and the aperture-size limit").
- Result: a free panel at matched particle velocity absorbs only $P_\text{abs}\sim I_0 A(A/\lambda^2)=I_0 A^2/\lambda^2$ (radiation efficiency $\eta\sim(ka)^2\sim A/\lambda^2$); at 100 Hz / 0.5 m², $A/\lambda^2\approx0.04$. This **size penalty is orthogonal to the open/sealed 50% axis** and afflicts open *and* sealed free panels alike (cured by size, baffle ≫λ, resonance, or near-field). Resonant escape to $\sigma_{\max}=3\lambda^2/8\pi\approx1.4$ m² costs excursion + bandwidth. Noted the panel is only marginally sub-wavelength at 100 Hz ($ka\approx0.7$).
- JOS's earlier `A/λ²` intuition in Discussion.md §4 was right for *this* regime; my earlier pushback had quoted the area-independent resonant $\sigma_{\max}$ and the baffled geometric $I_0A$ instead. Discussion.md §4 rewritten to lead with this analysis; §6 trimmed (σ_max moved to §4); §5/§7 reconciled.

## [2026-06-20] new concept | duct-acoustics-liner-modeling — E3 reading note + spec
- Created `concepts/duct-acoustics-liner-modeling.md`: the reading guide + modeling spec for E3 of the rooftop-fan roadmap (plane-wave lined-duct transfer-matrix model → TL; substrate for E4 passive liners and E5 FxLMS termination). Prompted by JOS asking what to read to get on top of the E3 tech.
- Reading list by tier: Munjal *Acoustics of Ducts and Mufflers* (the four-pole/TL reference); Rienstra-Hirschberg (free, staged); Cremer optimum via Tester 1973; Maa 1998 MPP; Howe 1979 + Jing-Sun 2000 bias-flow; Ingard 1953; Nelson-Elliott 1992; Ma-Sheng 2016.
- Key modeling decisions captured: output **TL not IL** for E3 (termination-independent); locally-reacting wall; and **the duct target impedance is the Cremer optimum, not $\rho c$** (corrects the free-field intuition from [[electroacoustic-absorbers]]). Includes a four-pole element library table (hard-wall duct, side-branch Helmholtz, lined section $k_x(\beta)$, bias-flow perforate) and the $\mathrm{TL}=20\log_{10}[\tfrac12|A+B/Y+CY+D|]$ output formula.
- Staged `incoming-pdfs/RienstraHirschberg_IntroToAcoustics.pdf` (author-hosted boek.pdf, free) with committed `incoming-pdfs/README.md` provenance. Maa AIP link returned an HTML paywall stub — left as a pending DOI.
- Added 9 references to `pending-sources.md` under a new "For [[duct-acoustics-liner-modeling]] (E3/E4/E5 build)" section; index Concepts count 16→17, date→2026-06-20.
- This wiki note is the spec the project-side E3 scaffold (extending `02_piston_absorber.py` / `03_helmholtz_absorber.py`) will build against; companion to `~/w/absorbers/RoadMap.md` §5.

## [2026-06-20] ingest | Maa 1998 MPP absorber → paper-maa-mpp-1998
- JOS supplied the Maa 1998 JASA MPP paper (AIP access) in `~/w/absorbers/papers/`. Ingested per convention: copied to `/l/dttd/MicroperforatedPanelAbsorber-Maa-JASA1998.pdf`, symlinked into `ANC-Stuff/`, `pdf2txt.py` → `raw/MicroperforatedPanelAbsorber-Maa-JASA1998.txt` (1204 lines; equations OCR-garbled but model reconstructed from the standard Maa theory + readable fragments).
- Distilled into `raw/SUMMARIES.md` (new "For duct-acoustics-liner-modeling" section) and `entities/source-papers.md` (new "Duct-acoustics & liner modeling (2026-06-20)" section, slug `paper-maa-mpp-1998`). Captured: relative impedance $z=r+j\omega m$ via perforate constant $k=d\sqrt f/10$; $r{=}1$ matched → $\alpha_0{=}1$; bandwidth/resistance tradeoff; deep-subwavelength $D/\lambda<1/4$ for LF (but still a $\lambda/4$-cavity resonator — 118 Hz wants deep/coiled cavity); viscous-vs-bias-flow perforate contrast.
- Promoted [[duct-acoustics-liner-modeling]]: Maa link now "Ingested →" cross-ref; added MPP row + surface-absorption formula to the element-library table.
- Bookkeeping: `raw/MANIFEST.md` row added; `index.md` catalog count 48→49 + new entity batch line; `pending-sources.md` Maa entry struck through as ingested.
- Still wanted for the E3/E4 build (flagged to JOS): **Munjal** *Acoustics of Ducts and Mufflers* (the four-pole/TL method — top priority, book/library) and **Jing & Sun 2000** (AIAA — codeable bias-flow perforate impedance; AIAA access ≠ AIP). Bias-flow *theory* is already covered by Howe 1979 intent + Eldredge-Dowling 2003 (catalog) + Galland/Betgen hybrid liners (catalog).
